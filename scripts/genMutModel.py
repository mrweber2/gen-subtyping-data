...
parser.add_argument('--csv',         required=False, action='store_true',       default=False, help="generate feature csv")
parser.add_argument('-co',           type=str, required=False, metavar='<str>',                help="* output.csv")
parser.add_argument('--donor',       required=False, action='store_true',       default=False, help="provide donor stats")
parser.add_argument('--save-trinuc', required=False, action='store_true',       default=False, help='save trinuc counts for ref')
args = parser.parse_args()
(REF, TSV, OUT_PICKLE, SAVE_TRINUC, CSV, SITE, DONOR, OUTcsv) = (args.r, args.m, args.o, args.save_trinuc, args.csv, args.C, args.donor, args.co)
...
#					[donor_id] = [splt[d_id]]
		...
		for line in f:
			...
			#if DONOR:
			#	if donor_id not in D_LIST:
			#		D_LIST.append(donor_id)
			#print D_LIST
			...
			if '-' not in allele_normal and '-' not in allele_tumor and chrStart == chrEnd:
				...
				#				key = trinuc_normal
				#				donor_.key_.count += 1
				#				if key not in donorDict:
				#					donorDict[key] = 1
				#				else:
				#					donorDict[key] += 1
				#			print donorDict
				#			donorDict.append(dict(zip(keys, )))
					trinuc_normal    = refSequence[chrStart-1] + allele_normal + refSequence[chrStart+1]
					trinuc_tumor     = refSequence[chrStart-1] + allele_tumor + refSequence[chrStart+1]
	...
	for k in sorted(TRINUC_TRANS_PROBS.keys()):
		...
		if CSV:
			c = open(OUTcsv, 'w')
			#c.write('m_id,d_id')
			for trinuc in TRINUC_TRANS_PROBS:
				trinuc = str(trinuc)
				trinuc.replace(', ','\>')
				c.write(str(trinuc))
				c.write(',')
			c.write('site\n')
			for trinuc in TRINUC_TRANS_PROBS:
				c.write(str(TRINUC_TRANS_PROBS[trinuc]))
				c.write(',')
			c.write(str(SITE))
			c.close()
